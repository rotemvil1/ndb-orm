...
class BooleanProperty(Property):
  ...
  def _db_set_value(self, v, value):
    ...
    v.boolean_value = value # scalar, just assign

  def _db_get_value(self, v):
    if v.WhichOneof('value_type') != 'boolean_value':
      return None
...
#     v.meaning = PROPERTY_GEORSS_POINT
  ...
  def _db_get_value(self, v):
    if v.WhichOneof('value_type') != 'geo_point_value':
      return None
    pv = v.geo_point_value
    return GeoPt(pv.latitude, pv.longitude)
...
class UserProperty(Property):
  ...
  def _db_get_value(self, v):
    if v.WhichOneof('value_type') != 'user_value':
      return None
  ...
  def _db_set_value(self, v, value):
    if not isinstance(value, datastore_types.BlobKey):
      raise TypeError('BlobKeyProperty %s can only be set to BlobKey values; '
                      'received %r' % (self._name, value))
    p.meaning = PROPERTY_BLOBKEY
    v.string_value = str(value)

  def _db_get_value(self, v):
    if v.WhichOneof('value_type') != 'string_value':
      return None
    return datastore_types.BlobKey(v.string_value)
...
#       return Key(pairs=pairs, app=app, namespace=namespace)
  ...
  def _db_set_value(self, v, value):
    ...
    if isinstance(value, str):
      ...
      v.string_value = value.encode('utf8')
      if not self._indexed:
        p.meaning = PROPERTY_TEXT
...
#     elif isinstance(value, Key): # TODO
...
#       v.meaning = PROPERTY_GEORSS_POINT
      ...
      pv.latitude = value.lat
      pv.longitude = value.lon
#     elif isinstance(value, users.User): #TODO
#       datastore_types.PackUser(p.name(), value, v)
...
#     ent._set_projection(projection)
  ...
  def _get_property_for(self, name, p, indexed=True, depth=0):
...
#     """
#     if self._projection:
#       raise BadRequestError('Cannot put a partial entity')
